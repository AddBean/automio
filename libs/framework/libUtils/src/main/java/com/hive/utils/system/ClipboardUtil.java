...
//    }
    ...
    private ClipboardUtil(Context context) {
        mContext = context;
        mClipboardManager = (ClipboardManager) GlobalApp.getApp().getSystemService(Context.CLIPBOARD_SERVICE);
...
//    }
    ...
    public boolean hasPrimaryClip() {
        try {
            return mClipboardManager != null && mClipboardManager.hasPrimaryClip();
    ...
    public String getClipText() {
        ...
        try {
            ClipData data = mClipboardManager == null ? null : mClipboardManager.getPrimaryClip();
            ClipDescription clipDescription = mClipboardManager == null ? null : mClipboardManager.getPrimaryClipDescription();
            if (data != null && clipDescription != null && clipDescription.hasMimeType(ClipDescription.MIMETYPE_TEXT_PLAIN)) {
                if (data.getItemAt(0) != null && data.getItemAt(0).getText() != null) {
                    return data.getItemAt(0).getText().toString();
                }
            }
        } catch (Exception e) {
            // java.lang.SecurityException: Calling uid does not own package
            e.printStackTrace();
        }
