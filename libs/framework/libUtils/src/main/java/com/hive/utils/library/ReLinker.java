...
class ReLinker {
	...
	private static File unpackLibrary(final Context context, final String libName) {
		File outputFile = new File(context.getDir(LIB_DIR, Context.MODE_PRIVATE), libName + getAppVersionCode(context));
        Log.i("loadLibrary","outputFile = " + outputFile.getAbsolutePath());

		if (outputFile.isFile()) {
			return outputFile;
		}

		final File cachedLibraryFile = new File(context.getCacheDir(), libName + getAppVersionCode(context));
	...
	private static ZipEntry findLibraryEntry(final String libName, final ZipFile zipFile) {
        ...
        ZipEntry entry = getEntry(libName, zipFile, Build.CPU_ABI);
        if(null == entry) {
            entry = getEntry(libName, zipFile, Build.CPU_ABI2);
        }
