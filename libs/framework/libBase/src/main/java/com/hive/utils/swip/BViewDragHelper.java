...
/**
 * BViewDragHelper is a utility class for writing custom ViewGroups. It offers a
...
public class BViewDragHelper {
    ...
    private static final String TAG = "BViewDragHelper";
    ...
    /**
     * Apps should use BViewDragHelper.create() to get a new instance. This will
     ...
     */
    private BViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
    ...
    /**
     * Factory method to create a new BViewDragHelper.
     *
     * @param forParent Parent view to monitor
     * @param cb        Callback to provide information and receive events
     * @return a new BViewDragHelper instance
     */
    public static BViewDragHelper create(ViewGroup forParent, Callback cb) {
        return new BViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new BViewDragHelper instance
     */
    public static BViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final BViewDragHelper helper = create(forParent, cb);
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant " + "of the BViewDragHelper's tracked parent view (" + mParentView + ")");
        }
     ...
     * (to the best of the BViewDragHelper's knowledge).
     * <p>
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_JUDGING) {
                    ...
                    if (mDragState == STATE_DRAGGING) mVelocityTracker.addMovement(ev);
    ...
    public void recordInitialMotion(MotionEvent ev) {
    ...
    private int checkTouchSlop(View child, float dx, float dy) {
        ...
        if (dx > mTouchSlop && Math.abs(dy) <= mTouchSlop) {
            if ((mTrackingEdges & EDGE_LEFT) != 0 || (mTrackingEdges & EDGE_RIGHT) != 0) {
        ...
        } else if (Math.abs(dy) > Math.abs(dx) && dy > 0) {//dx <= mTouchSlop && Math.abs(dy) > mTouchSlop
            if ((mTrackingEdges & EDGE_TOP) != 0 || (mTrackingEdges & EDGE_BOTTOM) != 0) {
                mDragState = STATE_DRAGGING;
                return 1;
            } else {
                mDragState = STATE_IDLE;
    ...
    private void releaseViewForPointerUp() {
        if (mVelocityTracker != null) {
            ...
            if (getViewDragState() == STATE_DRAGGING) dispatchViewReleased(xvel, yvel);
        }
    }
    ...
    private int getEdgeTouched(int x, int y) {
        if (x < mParentView.getLeft() + mEdgeSize) return EDGE_LEFT;
        if (y < mParentView.getTop() + mEdgeSize) return EDGE_TOP;
        if (x > mParentView.getRight() - mEdgeSize) return EDGE_RIGHT;
        if (y > mParentView.getBottom() - mEdgeSize) return EDGE_BOTTOM;
        //TODO changed this for full screen touch;
        return 0;
    }

    /**
     * A Callback is used as a communication channel with the BViewDragHelper
     ...
     * the BViewDragHelper with more information about the state of the parent
    ...
    public static abstract class Callback {
         ...
         * invokes one of these methods, the BViewDragHelper will enter
         ...
         * place and the BViewDragHelper will return to {@link #STATE_IDLE}.
         * </p>
         *
         ...
         * <p>
         * BViewDragHelper may call this method multiple times for the same view
