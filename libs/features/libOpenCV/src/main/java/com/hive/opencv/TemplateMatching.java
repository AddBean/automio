...
public class TemplateMatching {
    ...
    public static List<Match> fastTemplateMatching(Mat img, Mat template, int matchMethod, float weakThreshold, float strictThreshold, int maxLevel, int limit) {
        ...
        for (int level = maxLevel; level >= 0; level--) {
            // 自适应阈值：金字塔高层放宽阈值
            float levelWeakThreshold = weakThreshold - (level * 0.05f);  // 每层降低 0.05
            float levelStrictThreshold = strictThreshold - (level * 0.03f);
            levelWeakThreshold = Math.max(0.5f, levelWeakThreshold);  // 保证不低于 0.5
            levelStrictThreshold = Math.max(0.6f, levelStrictThreshold);
            ...
            if (previousMatchResult.isEmpty()) {
                ...
                getBestMatched(matchResult, currentTemplate, matchMethod, levelWeakThreshold, currentMatchResult, limit, null);
                ...
                for (Match match : previousMatchResult) {
                    ...
                    getBestMatched(matchResult, currentTemplate, matchMethod, levelWeakThreshold, currentMatchResult, limit, r);
            ...
            if (!currentMatchResult.isEmpty()) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (match.similarity >= levelStrictThreshold) {  // 使用自适应阈值
    ...
    private static Rect getROI(Point p, Mat src, Mat currentTemplate) {
        // 扩大搜索范围：从 1.5倍改为 2.5倍
        int x = (int) (p.x * 2 - currentTemplate.cols() / 2);  // 增加偏移范围
        x = Math.max(0, x);
        int y = (int) (p.y * 2 - currentTemplate.rows() / 2);
        y = Math.max(0, y);
        int w = (int) (currentTemplate.cols() * 2.5);  // 从 1.5 改为 2.5
        int h = (int) (currentTemplate.rows() * 2.5);
        if (x + w >= src.cols()) {
            w = src.cols() - x - 1;
        }
        if (y + h >= src.rows()) {
