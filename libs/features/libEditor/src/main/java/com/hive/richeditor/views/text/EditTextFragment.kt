...
import com.hive.richeditor.views.MenuHelper
 ...
 */
class EditTextFragment : BaseFragment(), IBackListener {
    private var mInitContentMd5: String? = null
    var mFile: File? = null
    private var hasSaved = false

    private var editor: EditTextView? = null
    ...
    fun checkInput() {
        if (TextUtils.isEmpty(editor?.getContent())) throw Exception(getStr(com.hive.i8n.R.string.edit_tips_file_not_edit))
    }

    fun loadFileText(file: File?, charset: String) {
        mFile = file
        view?.postDelayed({
            tv_title?.text = mFile?.name ?: getStr(com.hive.i8n.R.string.default_file_txt_name)
            try {
                Thread {
                    var content = ""
                    if (file != null) {
                        if (file.exists()) {
                            var text = FileUtils.readFile(file?.path, charset)
    ...
    private fun saveTo() {
        ...
        dialog.setEditText(getStr(com.hive.i8n.R.string.default_file_txt_name))
    ...
    private fun trySaveToLocal(closeActivity: Boolean) {
        try {
            checkInput()
            FileUtils.writeFile(mFile, editor?.getContent(), mCharset)
            hasSaved = true
    ...
    val isHasChanged: Boolean
        get() {
            return if (editor?.getContent() == null) {
                return false
            } else {
                !TextUtils.equals(mInitContentMd5, Md5Utils.string2md5(editor?.getContent() ?: ""))
            }
        }
    ...
    public override fun getLayoutId() = R.layout.edit_text_fragment
