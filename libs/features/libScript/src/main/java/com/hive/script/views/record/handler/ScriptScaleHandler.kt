...
class ScriptScaleHandler(recordView: IScriptRecordView) : ScriptRecordEventHandler(recordView) {
    override fun handleEvent(action: RecordResultAction, obj: Any?) {
        if (obj == null) {
            recordView.setViewState(
                recordView.getViewState()
                    .ofFalse(ScriptRecordViewManager.RecordViewType.SCALE_IN_OUT)
            )
            ScriptInsertManager.notifyInsertDismiss()
        } else {
            val cmd = obj as CmdPinchZoom
            showScaleInOutConfirmDialog(cmd)
        }
    }

    /**
     * 显示缩放确认对话框（统一通过 DialogScriptCardEdit 编辑）
     */
    private fun showScaleInOutConfirmDialog(cmd: CmdPinchZoom) {
