 ...
 * @date 6/16/21
 */
class CommonDelayEditView(context: Context) : BaseCommandEditCard(context), View.OnClickListener {
    ...
    override fun initView() {
        ...
        number_start_duration?.changedListener =
            FloatOptView.OnValueChangedListener { value ->
                cmd?.startDelay = value.toLong()
                if (cmd is CmdDelay) {
                    (cmd as CmdDelay).startDuration = value.toLong()
                }
            }
        number_end_duration?.changedListener =
            FloatOptView.OnValueChangedListener { value ->
                cmd?.endDelay = value.toLong()
                if (cmd is CmdDelay) {
                    (cmd as CmdDelay).endDuration = value.toLong()
                }
            }
    ...
    override fun onBindCommand(command: ScriptCommand) {
        cmd = command
        number_start_duration?.setNumber((cmd?.startDelay ?: 100).toFloat())
        number_end_duration?.setNumber((cmd?.endDelay ?: 100).toFloat())
    }


    override fun isSupportEdit() = true

    override fun getEditContentId() = R.layout.cmd_common_delay_card
