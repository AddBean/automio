// Copyright (c) 2026 jiadou
// SPDX-License-Identifier: MIT

package com.hive.script.views.dialog

import android.content.Context
import android.text.Editable
import android.text.TextWatcher
import android.view.Gravity
import android.view.View
import android.widget.EditText
import android.widget.TextView
import com.hive.script.R
import com.hive.script.views.widgets.BaseScriptDialog
import com.hive.utils.system.CommonUtils

/**
 *
 * @author jiadou
 * @date 6/11/21
 */
class DialogInputMessage(
    context: Context?,
    private var title: String,
    private var hint: String,
    private var txtHold: String? = null,
    private var inputType: Int = 0,
    private var checkInputFun: ((edit_text: EditText) -> Unit)?,
    private var confirmFun: ((dialog: DialogInputMessage, input: String) -> Unit)
) : BaseScriptDialog(context) {
    private lateinit var edit_text: EditText
    private var btn_submit: View? = null
    private var iv_close: View? = null
    private var tv_msg: TextView? = null
    private var tv_title: TextView? = null

    override fun initWindow() {
        btn_submit = findViewById(R.id.btn_submit)
        edit_text = findViewById(R.id.edit_text)
        iv_close = findViewById(R.id.iv_close)
        tv_msg = findViewById(R.id.tv_msg)
        tv_title = findViewById(R.id.tv_title)

        iv_close?.setOnClickListener {
            dismiss()
        }

        btn_submit?.setOnClickListener {
            try {
                checkInputFun?.invoke(edit_text)
                confirmFun.invoke(this, edit_text.text.toString().trim())
                dismiss()
            } catch (e: Exception) {
                tv_msg?.visibility = VISIBLE
                tv_msg?.text = e.message
            }
        }
        if (inputType > 0) {
            edit_text.inputType = inputType
        }
        edit_text?.addTextChangedListener(object : TextWatcher {
            override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {
                tv_msg?.visibility = GONE
            }

            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {

            }

            override fun afterTextChanged(s: Editable?) {
                tv_msg?.visibility = GONE
            }

        })
        edit_text?.requestFocus()
        edit_text?.post {
            CommonUtils.openKeyboard(edit_text)
            tv_title?.text = title
            edit_text?.hint = hint
            txtHold?.let {
                edit_text?.setText(it)
                edit_text.setSelection(0, it.length)
            }
        }

    }

    fun setMultiLine(): DialogInputMessage {
        edit_text?.isSingleLine = false
        edit_text?.setLines(5)
        edit_text?.gravity = Gravity.LEFT or Gravity.TOP
        edit_text?.layoutParams?.apply {
            height = CommonUtils.dp2Px(100)
            edit_text?.layoutParams = this
        }
        return this
    }


    override fun enableFadeAnimation() = true

    override fun isTouchOutsideDismissed() = true

    override fun getWindowLayoutId() = R.layout.dialog_input_message
}