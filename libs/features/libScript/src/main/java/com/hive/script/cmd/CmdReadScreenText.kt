 ...
 * @date 6/8/21
 */
@AutoCmdRegister(type = IDS.CmdReadScreenText, name = "readScreenText")
class CmdReadScreenText : ScriptCommand(), ScriptRegularInterface {

    var targetParamId = ScriptParamEnv.getDefaultParam()?.getFullId()

    var resultText: String? = null

    var resultFull: OcrResult? = null

    override fun onExecute(): CmdExecuteResult {
        val result = ScriptEventHelper.get().tryReadOcrTextInSync(
            limitRect
        )
        resultText = result?.first
        resultFull = result?.second
        ...
        ScriptInterpreterObserver.notifyLogger(
            ...
            com.hive.i8n.R.string.sc_read_text.string(result ?: "")
        )
        writeParam(targetParamId, resultText)
        ScriptThreadManager.delay(ScriptConst.Cmd_Default_Base)
        return CmdExecuteResult.success(resultFull)
    }
    ...
    override fun getCommandName() = GlobalApp.getString(com.hive.i8n.R.string.cmd_name_read_screen_text)

    override fun getCommandDescribe() = GlobalApp.getString(com.hive.i8n.R.string.cmd_des_read_screen_text)
    ...
    companion object {
        fun createCommand(paramFullId: String?) = CmdReadScreenText().apply {
