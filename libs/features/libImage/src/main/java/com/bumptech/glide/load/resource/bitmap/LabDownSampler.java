 ...
 */
public final class LabDownSampler {
    ...
    public static final Option<Boolean> AUTO_LIMIT_DIMENSIONS =
            Option.memory("com.bumptech.glide.load.resource.bitmap.Downsampler.MAX", true);
    ...
    //16:9
    private static final int MAX_DECODE_PICTURE_SIZE = 360 * 640;
    ...
    private static boolean mLimitImageSize = true;

    public LabDownSampler(List<ImageHeaderParser> parsers, DisplayMetrics displayMetrics,
    ...
    public Resource<Bitmap> decode(InputStream is, int requestedWidth, int requestedHeight,
        ...
        mLimitImageSize = options.get(AUTO_LIMIT_DIMENSIONS);
    ...
    private static Bitmap decodeStream(InputStream is, BitmapFactory.Options options,
        ...
        try {
            if (mLimitImageSize && !options.inJustDecodeBounds) {

                if (mBitmapMax == null) {
                    mBitmapMax = ImageTypeHelper.getGLESTextureLimit();
                    if (DLog.isDebug()) {
                        DLog.e("OpenGLRenderer2", "max=h=" + mBitmapMax[0] + ";w=" + mBitmapMax[1]);
                    }
                }

                int max = mBitmapMax[0];

                if (options.outHeight > max || options.outWidth > max) {
                    BitmapRegionDecoder decoder = BitmapRegionDecoder.newInstance(is, true);
                    if (rect == null) {
                        rect = new Rect(0, 0, Math.min(options.outWidth, max), Math.min(options.outHeight, max));
                    }
                    rect.left = 0;
                    rect.top = 0;
                    rect.bottom = Math.min(options.outHeight, max);
                    rect.right = Math.min(options.outWidth, max);
                    result = decoder.decodeRegion(rect, options);
                }
            }
            ...
            if (DLog.isDebug() && result != null && !options.inJustDecodeBounds) {
                if (result.getHeight() > 1280) {
                    int w = Math.min(result.getWidth(), 720);
                    int h = Math.min(result.getHeight(), 640);
                    Bitmap bitmap = bitmapPool.get(w, h, options.inPreferredConfig);
                    ...
                    result = bitmap;
                    if (DLog.isDebug()) {
                        DLog.e("OpenGLRenderer", "last==" + result.getHeight() + ";w=" + result.getWidth());
                    }
                }
            }
