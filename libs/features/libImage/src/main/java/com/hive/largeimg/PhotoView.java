...
public class PhotoView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    // disableEvent listener;
    private OnDisableEventListener onDisableEventListener;
    ...
    public PhotoView(Context context, AttributeSet attr) {
        ...
        if (attr != null) {
            TypedArray typedAttr = getContext().obtainStyledAttributes(attr, R.styleable.PhotoView);
            if (typedAttr.hasValue(R.styleable.PhotoView_assetName)) {
                String assetName = typedAttr.getString(R.styleable.PhotoView_assetName);
                if (assetName != null && assetName.length() > 0) {
                    setImage(ImageSource.asset(assetName).tilingEnabled());
                }
            }
            if (typedAttr.hasValue(R.styleable.PhotoView_src)) {
                int resId = typedAttr.getResourceId(R.styleable.PhotoView_src, 0);
                if (resId > 0) {
                    setImage(ImageSource.resource(resId).tilingEnabled());
                }
            }
            if (typedAttr.hasValue(R.styleable.PhotoView_panEnabled)) {
                setPanEnabled(typedAttr.getBoolean(R.styleable.PhotoView_panEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.PhotoView_zoomEnabled)) {
                setZoomEnabled(typedAttr.getBoolean(R.styleable.PhotoView_zoomEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.PhotoView_quickScaleEnabled)) {
                setQuickScaleEnabled(typedAttr.getBoolean(R.styleable.PhotoView_quickScaleEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.PhotoView_tileBackgroundColor)) {
                setTileBackgroundColor(typedAttr.getColor(R.styleable.PhotoView_tileBackgroundColor, Color.argb(0, 0, 0, 0)));
            }

            if (typedAttr.hasValue(R.styleable.PhotoView_isFullScreen)) {
                mIsFullScreen = typedAttr.getBoolean(R.styleable.PhotoView_isFullScreen, false);
            }
            typedAttr.recycle();
        }
    ...
    public static void setPreferredBitmapConfig(Bitmap.Config preferredBitmapConfig) {
        PhotoView.preferredBitmapConfig = preferredBitmapConfig;
    }
    ...
    public final void setImage(@NonNull ImageSource imageSource, ImageSource previewSource, ImageViewState state) {
        ...
        if (imageSource.getBitmap() != null && imageSource.getSRegion() != null) {
            ...
            if (imageSource.getTile() || sRegion != null) {
                ...
                TilesInitTask task = new TilesInitTask(this, getContext(), regionDecoderFactory, uri, mIsFullScreen);
                execute(task);
            } else {
    ...
    private void setGestureDetector(final Context context) {
        this.detector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
                if (onDisableEventListener != null) {
                    onDisableEventListener.onDisableEvent(true);
                }
    ...
    private static class TilesInitTask extends AsyncTask<Void, Void, int[]> {
        ...
        TilesInitTask(PhotoView view, Context context, DecoderFactory<? extends ImageRegionDecoder> decoderFactory, Uri source, boolean isFullScreen) {
        ...
        protected void onPostExecute(int[] xyo) {
            ...
            if (view != null) {
                if (decoder != null && xyo != null && xyo.length == 3) {
                    ...
                    int w = SystemProperty.getScreenWidth(context);
                    int h = SystemProperty.getScreenHeight(context) - (isFullScreen ? 0 : SystemProperty.getStatusBarHeight(context));
                    if (w > 0 && h > 0 && xyo[1] > h) {
                        int maxW = Math.min(w, Math.max(w, xyo[0]));
                        view.setScaleAndCenter((1.f * maxW) / xyo[0], new PointF(0, 0));
                    }
    ...
    private static class TileLoadTask extends AsyncTask<Void, Void, Bitmap> {
        ...
        protected void onPostExecute(Bitmap bitmap) {
            final PhotoView photoView = viewRef.get();
            ...
            if (photoView != null && tile != null) {
                ...
                } else if (exception != null && photoView.onImageEventListener != null) {
                    photoView.onImageEventListener.onTileLoadError(exception);
                }
            }
        }
    }
    ...
    private static class BitmapLoadTask extends AsyncTask<Void, Void, Integer> {
        ...
        protected void onPostExecute(Integer orientation) {
            PhotoView photoView = viewRef.get();
            if (photoView != null) {
                if (bitmap != null && orientation != null) {
                    if (preview) {
                        photoView.onPreviewLoaded(bitmap);
                    } else {
                        photoView.onImageLoaded(bitmap, orientation, false);
                    }
                } else if (exception != null && photoView.onImageEventListener != null) {
                    if (preview) {
                        photoView.onImageEventListener.onPreviewLoadError(exception);
                    } else {
                        photoView.onImageEventListener.onImageLoadError(exception);
                    }
                }
            }
        }
    ...
    public void setOnDisableEventListener(OnDisableEventListener listener) {
        this.onDisableEventListener = listener;
    }
    ...
    private void sendStateChanged(float oldScale, PointF oldVTranslate, int origin) {
        ...
        if (isChanged && onDisableEventListener != null) {
            onDisableEventListener.onDisableEvent(true);
        }
    }
    ...
    public interface OnDisableEventListener {
        void onDisableEvent(boolean disabled);
    }
