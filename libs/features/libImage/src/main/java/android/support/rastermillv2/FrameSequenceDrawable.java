...
public class FrameSequenceDrawable extends Drawable implements Animatable, Runnable {
    ...
    public int getDuration() {
        return mFrameSequence != null ? mFrameSequence.getDuration() : 0;
    }
    ...
    private Runnable mFinishedCallbackRunnable = new Runnable() {
        ...
        public void run() {
            synchronized (mLock) {
                ...
                mState = 0;
                scheduleSelf(FrameSequenceDrawable.this, 100);
            }
    ...
    public FrameSequenceDrawable(FrameSequence frameSequence, BitmapProvider bitmapProvider) {
        ...
        try {
            if (mFirstBitmap == null) {
                mFirstBitmap = acquireAndValidateBitmap(bitmapProvider, width, height);
                int[] pixels = new int[width * height];
                mFrontBitmap.getPixels(pixels, 0, width, 0, 0, width, height);
                mFirstBitmap.setPixels(pixels, 0, width, 0, 0, width, height);
            }
        } catch (Throwable t) {
    ...
    public void destroy() {
        ...
        if (mFirstBitmap != null) {
            mBitmapProvider.releaseBitmap(mFirstBitmap);
            mFirstBitmap = null;
        }
    }
    ...
    public void draw(Canvas canvas) {
        synchronized (mLock) {
            ...
            if (isRunning() && mState == STATE_READY_TO_SWAP) {
                ...
                if (mNextFrameToDecode == mFrameSequence.getFrameCount() - 1) {
                    mCurrentLoop++;
                    if ((mLoopBehavior == LOOP_FINITE && mCurrentLoop == mLoopCount) || (mLoopBehavior == LOOP_DEFAULT && mCurrentLoop == mFrameSequence.getDefaultLoopCount())) {
        ...
        if (mCircleMaskEnabled) {
            ...
            mTempRectF.set((bitmapWidth - scaledDiameterX) / 2.0f, (bitmapHeight - scaledDiameterY) / 2.0f, (bitmapWidth + scaledDiameterX) / 2.0f, (bitmapHeight + scaledDiameterY) / 2.0f);
            ...
            if (isRunning()) {
                ...
                if (mFirstBitmap != null && !mFirstBitmap.isRecycled()) {
                    canvas.drawBitmap(mFirstBitmap, mSrcRect, getBounds(), mPaint);
                }
            }
        }
    }
    ...
    public void stop() {
        if (isRunning()) {
            unscheduleSelf(this);
            scheduleSelf(FrameSequenceDrawable.this, 0);
        }
    }
    ...
    public boolean setVisible(boolean visible, boolean restart) {
        ...
        //https://zhuanlan.zhihu.com/p/36194127
        // 7.0及以上特殊处理 暂时没找到其他好办法
