...
public class FrameSequence {
    ...
    private static native long nativeGetFrame(long nativeState, int frameNr, Bitmap output, int previousFrameNr);
     ...
     */
    @Keep
    private FrameSequence(long nativeFrameSequence, int width, int height, boolean opaque, int frameCount, int defaultLoopCount, int duration) {
...
//        Arrays.fill(buffer, (byte) 0);
        FrameSequence frameSequence = nativeDecodeStream(stream, new byte[16 * 1024]);
//        BUFFER_REF.set(buffer);
    ...
    public static boolean isSupport(InputStream stream, boolean resetStream) throws Exception {
        ...
        if (resetStream && stream instanceof FileInputStream) {
            FileChannel channel = ((FileInputStream) stream).getChannel();
            channel.position(0);
            channel.close();
        }
        if (resetStream && stream.markSupported()) {
            try {
                stream.reset();
            } catch (IOException e) {
                throw e;
